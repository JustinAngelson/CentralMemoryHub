...
These are the core tools for cross-agent organizational memory:
semantic search, store, and retrieve for both unstructured and structured memories.

Reference implementation — Replit Agent should use this as the pattern for
organization_tools.py and agent_tools.py.
"""
import json
import uuid
import logging
...
# --- Database and Pinecone access ---
# These imports assume the existing CMH project structure.
# Adjust paths if needed based on actual project layout.
...
class StoreMemoryInput(BaseModel):
    """Input for storing unstructured memory."""
    model_config = ConfigDict(str_strip_whitespace=True, extra="forbid")
    ...
    content: str = Field(
        ...
        min_length=1,
        max_length=10000,
        description="Content to store in organizational memory",
    )


class GetMemoryInput(BaseModel):
    """Input for retrieving memory by ID."""
    model_config = ConfigDict(str_strip_whitespace=True, extra="forbid")
...
class StoreStructuredInput(BaseModel):
    """Input for storing structured memory."""
    model_config = ConfigDict(str_strip_whitespace=True, extra="forbid")
...
def register_memory_tools(mcp: FastMCP) -> None:
    ...
    async def cmh_store_memory(params: StoreMemoryInput) -> str:
        ...
        Use this for any information that should persist across sessions and be retrievable
